...
[`src/index.ts`](src/index.ts) is the runtime composition root.

At startup it:

1. Loads and validates environment configuration.
2. Creates the logger.
3. Opens SQLite and runs Drizzle migrations from [`src/db/migrations/`](src/db/migrations).
...
[`src/server/routes.ts`](src/server/routes.ts) exposes:

- `GET /health`
- `POST /webhook`

[`src/server/middleware.ts`](src/server/middleware.ts) adds:
...
2. Parse the payload with Zod schemas in [`src/events/parser.ts`](src/events/parser.ts).
3. Ignore unsupported or intentionally ignored events.
4. Dedupe using `Idempotency-Key` or `X-Gitlab-Webhook-UUID`.
5. Route the event in [`src/events/router.ts`](src/events/router.ts).
...
Mention parsing lives in [`src/events/mention.ts`](src/events/mention.ts). Only the first token after mention stripping can override the default agent.

## Persistence

SQLite is the only persistence layer.

[`src/db/schema.ts`](src/db/schema.ts) defines:
...
[`src/jobs/queue.ts`](src/jobs/queue.ts) stores queued work with:
...
[`src/sessions/manager.ts`](src/sessions/manager.ts) persists resumable agent sessions for issue and MR conversations. Review jobs track review context separately from conversational MR note sessions.

## Worker Model

[`src/jobs/worker.ts`](src/jobs/worker.ts) processes one queue item at a time per lane.
...
5. Spawns the selected agent through [`src/agents/runner.ts`](src/agents/runner.ts).
...
[`src/gitlab/service.ts`](src/gitlab/service.ts) is intentionally narrow. It only handles:
...
[`src/agents/runner.ts`](src/agents/runner.ts) dispatches to per-agent adapters:

- [`src/agents/claude.ts`](src/agents/claude.ts)
- [`src/agents/codex.ts`](src/agents/codex.ts)
- [`src/agents/gemini.ts`](src/agents/gemini.ts)
...
- [`tests/integration/webhook-queue.test.ts`](tests/integration/webhook-queue.test.ts)
